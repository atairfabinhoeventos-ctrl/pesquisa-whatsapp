// ==================================================================
// ARQUIVO: index.js (Versão Final com Correção de Payload da Meta)
// ==================================================================
...
async function enviarMensagem(para, mensagem) { console.log(`[ENVIO MSG] Para: ${para}`); try { await axios.post(`https://graph.facebook.com/v19.0/${PHONE_NUMBER_ID}/messages`, { messaging_product: "whatsapp", to: para, ...mensagem }, { headers: { 'Authorization': `Bearer ${WHATSAPP_TOKEN}` } }); } catch (error) { console.error("Erro ao enviar mensagem:", error.response ? error.response.data : error.message); } }
function setConversationTimeout(contato) { clearConversationTimeout(contato); userTimeouts[contato] = setTimeout(() => { delete userState[contato]; delete userTimeouts[contato]; console.log(`[TIMEOUT] Conversa com ${contato} encerrada.`); enviarMensagem(contato, { type: 'text', text: { body: '⏳ Sua sessão foi encerrada por inatividade. Envie uma nova mensagem se quiser recomeçar. 👋' } }); }, CONVERSATION_TIMEOUT); }
...
app.get('/webhook', (req, res) => { if (req.query['hub.mode'] === 'subscribe' && req.query['hub.verify_token'] === VERIFY_TOKEN) { res.send(req.query['hub.challenge']); console.log("Webhook verificado com sucesso!"); } else { res.sendStatus(403); } });
...
app.post('/webhook', async (req, res) => {
    try {
        ...
        if (isAdmin) {
            ...
            await enviarMensagem(contato, { type: 'text', text: { body: `Olá, Administrador! 👋 Aqui está seu relatório de desempenho:\n\n${relatorioFormatado}` } });
            res.sendStatus(200);
            return;
        }
        ...
        if (state) {
            if (state.stage === 'aguardandoCPF') {
                ...
                if (!cpfFormatado) {
                    await enviarMensagem(contato, { type: 'text', text: { body: '❌ CPF inválido. Por favor, digite apenas os 11 números do seu CPF.' } });
                    setConversationTimeout(contato); return;
                }
                state.data.cpf = cpfFormatado;
                state.stage = 'aguardandoConfirmacaoCPF';
                const interactive = { type: "button", body: { text: `📄 O CPF digitado foi: *${cpfFormatado}*. Está correto?` }, action: { buttons: [{ type: "reply", reply: { id: "sim_cpf", title: "👍 Sim" } }, { type: "reply", reply: { id: "nao_cpf", title: "✏️ Não" } }]}};
                await enviarMensagem(contato, { type: "interactive", interactive });
                ...
                if (resposta === 'sim_cpf' || ['sim', 's'].includes(resposta)) {
                    ...
                    await enviarMensagem(contato, { type: 'text', text: { body: '👍 Ótimo! Agora, por favor, digite seu *Nome Completo*.' } });
                    setConversationTimeout(contato);
                } else {
                    state.stage = 'aguardandoCPF';
                    await enviarMensagem(contato, { type: 'text', text: { body: 'Ok, vamos tentar de novo. Por favor, digite seu CPF novamente.' } });
                    setConversationTimeout(contato);
                }
            } else if (state.stage === 'aguardandoNome') {
                state.data.nome = textoMsg;
                state.stage = 'aguardandoTelefone';
                await enviarMensagem(contato, { type: 'text', text: { body: '✅ Nome registrado. Para finalizar, digite seu número de *telefone com DDD*.' } });
                setConversationTimeout(contato);
                ...
                await enviarMensagem(contato, { type: 'text', text: { body: '🎉 Cadastro finalizado com sucesso! Vou verificar se há alguma pesquisa para você.' } });
                ...
                if (!isNaN(nota) && nota >= 0 && nota <= 10) {
                    ...
                    if (pesquisasRestantes.length > 0) {
                        ...
                        const interactive = { type: "button", body: { text: `✅ Avaliação registrada! Notamos que você tem mais pesquisas pendentes. Deseja avaliar outro evento agora?` }, action: { buttons: [{ type: "reply", reply: { id: "sim_continuar", title: "👍 Sim, por favor" } }, { type: "reply", reply: { id: "nao_continuar", title: "👎 Não, obrigado" } }]}};
                        await enviarMensagem(contato, { type: "interactive", interactive });
                        setConversationTimeout(contato);
                    } else {
                        await enviarMensagem(contato, { type: 'text', text: { body: `✅ Muito obrigado! Todas as suas pesquisas foram concluídas. ✨${footer}` } });
                    }
                } else {
                    await enviarMensagem(contato, { type: 'text', text: { body: '❌ Ops! Por favor, envie apenas um número de 0 a 10. 😉' } });
                    setConversationTimeout(contato);
                }
                ...
                if (!isNaN(escolha) && escolha > 0 && escolha <= state.data.length) {
                    ...
                    await enviarMensagem(contato, { type: 'text', text: { body: `Ótimo! 👍 Para o evento "${eventoEscolhido.NomeEvento}", qual nota de 0 a 10 você daria para o líder *${eventoEscolhido.NomeLider}*?` } });
                    setConversationTimeout(contato);
                } else {
                    await enviarMensagem(contato, { type: 'text', text: { body: `❌ Por favor, responda com um número válido entre 1 e ${state.data.length}.` } });
                    setConversationTimeout(contato);
                }
                ...
                if (resposta === 'sim_continuar' || ['sim', 's'].includes(resposta)) {
                    ...
                    delete userState[contato];
                    await enviarMensagem(contato, { type: 'text', text: { body: `Tudo bem! Agradecemos seu tempo. Tenha um ótimo dia! 👋${footer}` } });
                }
            }
        } else {
            const doc = await loadSpreadsheet();
            ...
            if (usuarioCadastrado) {
                ...
                await enviarMensagem(contato, { type: 'text', text: { body: msgBoasVindas } });
                setConversationTimeout(contato);
            }
        }
    } catch (error) {
