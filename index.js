// ==================================================================
// ARQUIVO: index.js (Versão Definitiva para Servidor VM com Baileys)
// ==================================================================
        ...
        try {
            ...
            if (state) {
                if (state.stage === 'aguardandoCPF') {
                    ...
                    if (!cpfFormatado) { await sock.sendMessage(remoteJid, { text: '❌ CPF inválido. Por favor, digite apenas os 11 números do seu CPF.' }); setConversationTimeout(contato, remoteJid); return; }
...
app.get('/api/dados', async (req, res) => {
    try {
        ...
        const sheetEventos = doc.sheetsByTitle['Eventos'];
        const rows = await sheetEventos.getRows();
        const dados = rows.map(row => ({ CPF: row['CPF (xxx.xxx.xxx-xx)'], NomeEvento: row.NomeEvento, NomeLider: row.NomeLider, PesquisaEnviada: row.PesquisaEnviada, Nota: row.Nota, DataResposta: row.DataResposta, }));
        res.json(dados);
    } catch (error) {
...
app.get('/api/estatisticas', async (req, res) => {
    try {
        ...
        const estatisticasLideres = ranking.reduce((acc, lider) => { acc[lider.lider] = { media: lider.media, totalRespostas: lider.totalVotos }; return acc; }, {});
        res.json({ totalCadastros, totalRespondidas, estatisticasLideres: estatisticasLideres });
    } catch (error) {
